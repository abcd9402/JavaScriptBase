...
// 2) IdolGroup 클래스는 아이돌 그룹 이름과 멤버 정보를 리스트로 들고있다.(name 프로퍼티, members 프로퍼티)
class IdolGroup{
    name;
    members;
...
// 3) Idol 클래스는 아이돌 이름과 출생년도 정보를 들고있다.(name 프로퍼티, year 프로퍼티)
class Idol{
    name;
    year;
...
const cIveMembers = iveMembers.map((e)=>{
   return  new FemaleIdol(e.name,e.year)})
console.log(cIveMembers);

const cBtsMembers = btsMembers.map((e)=>
    new MaleIdol(e['name'],e['year'])
);
